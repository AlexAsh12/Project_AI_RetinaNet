# -*- coding: utf-8 -*-
"""assignment1_RetinaNet.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-L77aHykbwtgF6qaXk7uTd0owclgYDm4
...
class Augmenter(object):
    def __call__(self, sample, flip_x=0.5):
        ...
        if np.random.rand() < flip_x:
            ...
            image = image[:, ::-1, :]       #horizontal flipping
            x1 = annots[:, 0].copy()
            x2 = annots[:, 2].copy()
...
def VOC_download(root_dir, year="2007"):
    ...
    urls = {
        "2007": {
            "trainval": "https://thor.robots.ox.ac.uk/pascal/VOC/voc2007/VOCtrainval_06-Nov-2007.tar",
    ...
    with tarfile.open(trainval_download_path, "r") as tar:
        tar.extractall(root_dir, filter='data') # Added filter='data'
...
class VOCDataset(VOCDetection):
    ...
    def __getitem__(self, idx):
        ...
        for obj in root.iter('object'):
            ...
            class_idx = self.class_to_ind[class_name]
            ...
            annots.append([x1,y1,x2,y2, class_idx])
...
# Remove existing VOCdevkit directory to force a fresh download
voc_devkit_path = os.path.join(path2data, 'VOCdevkit')
if os.path.exists(voc_devkit_path):
    print(f"Removing existing VOCdevkit at {voc_devkit_path}")
    shutil.rmtree(voc_devkit_path)
...
# Initializing the training DataLoader
sampler = AspectRatioBasedSampler(dataset_train, batch_size=BATCH_SIZE, drop_last=False)
dataloader_train = DataLoader(dataset_train, num_workers=2, batch_sampler=sampler, collate_fn=collater)

sampler_val = AspectRatioBasedSampler(dataset_val, batch_size=1, drop_last=False)
dataloader_val = DataLoader(dataset_val, num_workers=2, batch_sampler=sampler_val, collate_fn=collater)
# [END]
...
class PyramidFeatures(nn.Module):
    def __init__(self, C3_size, C4_size, C5_size, feature_size=256):
        ...
        # Side connections (1x1 conv to reduce to feature_size)
        ...
        # Additional layers P6 and P7 for very large objects
    ...
    def forward(self, inputs):
        ...
        # P5 is calculated directly from C5
        P5_x = self.P5_1(C5)
        P5_upsampled_x = nn.functional.interpolate(P5_x, size=(C4.shape[2], C4.shape[3]), mode='nearest')
        P5_x = self.P5_2(P5_x)

        # P4 combines the upsampled version of P5 with the side connection of C4
        P4_x = self.P4_1(C4)
        P4_x = P4_x + P5_upsampled_x
        P4_upsampled_x = nn.functional.interpolate(P4_x, size=(C3.shape[2], C3.shape[3]), mode='nearest')
        P4_x = self.P4_2(P4_x)

        # P3 combines the upsampled version of P4 with the side connection from C3
        P3_x = self.P3_1(C3)
        P3_x = P3_x + P4_upsampled_x
        P3_x = self.P3_2(P3_x)

        # P6 and P7 (Downsampling)
        P6_x = self.P6(C5)
        P7_x = self.P7(torch.nn.functional.relu(P6_x))
...
class RegressionModel(nn.Module):
    ...
    def forward(self, x):
        ...
        # We are reorganizing to get (Batch, All_Anchors, 4)
...
class ClassificationModel(nn.Module):
    def __init__(self, num_features_in, num_anchors=9, num_classes=20, feature_size=256):
        super(ClassificationModel, self).__init__()

        self.num_classes = num_classes
        self.num_anchors = num_anchors

        # We use 4 convolutional layers, just like in the RegressionModel
        self.conv1 = nn.Conv2d(num_features_in, feature_size, kernel_size=3, padding=1)
        self.act1 = nn.ReLU()
        ...
        # The output layer: num_anchors * num_classes
        # Each anchor must have a score for each class
    ...
    def forward(self, x):
        ...
        # From (Batch, Anchors*Classes, H, W) to (Batch, H*W*Anchors, Classes)
...
class BBoxTransform(nn.Module):
    def __init__(self, mean=None, std=None):
        ...
        if mean is None:
            self.register_buffer('mean', torch.from_numpy(np.array([0, 0, 0, 0]).astype('float32')))
        else:
            self.register_buffer('mean', mean)
        if std is None:
            self.register_buffer('std', torch.from_numpy(np.array([0.1, 0.1, 0.2, 0.2]).astype('float32')))
        else:
            self.register_buffer('std', std)

    def forward(self, boxes, deltas):
        # boxes: anchors [x1, y1, x2, y2]
        # deltas: predictions of RegressionModel [dx, dy, dw, dh]
        ...
        pred_boxes = torch.zeros(deltas.shape).to(deltas.device)
...
class Anchors(nn.Module):
    ...
    def forward(self, image):
        ...
        for i in range(len(self.pyramid_levels)):
            # Generate basic anchors for a position
            base_anchors = self.generate_anchors(base_size=self.sizes[i], ratios=self.ratios, scales=self.scales)

            # Calculate the size of the feature map at this level
            feat_h = int(np.ceil(image_shape[0] / self.strides[i]))
            feat_w = int(np.ceil(image_shape[1] / self.strides[i]))

            # Shift these anchors across the entire grid
            shift_x = (torch.arange(0, feat_w) + 0.5) * self.strides[i]
            shift_y = (torch.arange(0, feat_h) + 0.5) * self.strides[i]
            ...
            # Add shifts to the base anchors
        ...
        return torch.cat(all_anchors, dim=0).unsqueeze(0).to(image.device)

    def generate_anchors(self, base_size, ratios, scales):
        num_anchors = len(ratios) * len(scales)
        anchors = torch.zeros((num_anchors, 4))

        # We combine each ratio with each scale
        # [x1, y1, x2, y2] centered at (0,0)
        anchor_idx = 0
        for scale in scales:
            for ratio in ratios:
                ...
                anchors[anchor_idx, 0] = -w / 2
                anchors[anchor_idx, 1] = -h / 2
                anchors[anchor_idx, 2] = w / 2
                anchors[anchor_idx, 3] = h / 2
                anchor_idx += 1
        return anchors
    ...
    #shape: The shape of the feature map (H, W)
    #stride: The stride (e.g., 8, 16, 32...) corresponding to the pyramid level
    #anchors: The base anchors [num_anchors, 4]

        # Create x and y coordinates for each cells
        # we add 0.5 to be center on the pixel cell
        shift_x = (torch.arange(0, shape[1]) + 0.5) * stride
        shift_y = (torch.arange(0, shape[0]) + 0.5) * stride

        # create a grid of coordinates (Meshgrid)
        shift_y, shift_x = torch.meshgrid(shift_y, shift_x, indexing='ij')

        # 3. Flatten and stack to obtain [H*W, 4]
        # We repeat x and y to cover [x1, y1, x2, y2]
        shifts = torch.stack([
            shift_x.reshape(-1), shift_y.reshape(-1),
            shift_x.reshape(-1), shift_y.reshape(-1)
        ], dim=1)

        # Apply offsets to the base anchors
        # K = number of positions in the grid, A = number of anchors per position
        K = shifts.shape[0]
        A = anchors.shape[0]

        # We convert the anchors to (1, A, 4) and the shifts to (K, 1, 4)
        # The addition is broadcast to yield (K, A, 4)
        all_anchors = anchors.view(1, A, 4) + shifts.view(K, 1, 4)

        # Returns a flat tensor [K*A, 4]
...
class ResNet(nn.Module):
    def __init__(self, num_classes, block, layers):
        ...
        # Inititialization of the Subnets
        self.anchors = Anchors() # Moved this line before its first use
        num_anchors = len(self.anchors.ratios) * len(self.anchors.scales)  # = 9

        self.regressionModel = RegressionModel(num_features_in=256, num_anchors=num_anchors)
        self.classificationModel = ClassificationModel(num_features_in=256, num_anchors=num_anchors, num_classes=num_classes)
...
for epoch_num in range(5):
    ...
    for iter_num, data in enumerate(dataloader_train):
        ...
        # Retrieve the data from the dictionary and send it to the appropriate device (CPU or GPU)
        # we need : device = torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
!ls -F /content/drive/MyDrive/checkpoints
...
for epoch_num in range(5):
    ...
    for iter_num, data in enumerate(dataloader_train):
        ...
        # 1. GPU/CPU training and gradient resetting
        # Load the images and annotations from the ‘data’ dictionary
        imgs = data['img'].to(device).float()
        annots = data['annot'].to(device)

        optimizer.zero_grad()
        classification_loss, regression_loss = retinanet_FL([imgs, annots])
        loss = classification_loss.mean() + regression_loss.mean()
        ...
        print('Epoch: {} | Iteration: {} | Classification loss: {:1.5f} | Regression loss: {:1.5f}'.format(epoch_num, iter_num, classification_loss.detach().item(), regression_loss.detach().item()))
    ...
    torch.save({
        'epoch': epoch_num,
        'model_state_dict': retinanet_FL.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'loss': loss,
    }, f'/content/drive/MyDrive/checkpoints/retinanet_epoch_{epoch_num}.pt')
...
retinanet_FL = resnet101(num_classes=20)

# Reload the model after modifying BBoxTransform
checkpoint = torch.load('/content/drive/MyDrive/checkpoints/retinanet_epoch_1.pt') # ou epoch_3 selon tes fichiers

# Use strict=False to ignore missing keys (for the newly registered buffers)
retinanet_FL.load_state_dict(checkpoint['model_state_dict'], strict=False)
retinanet_FL = retinanet_FL.cuda()
retinanet_FL.eval()

print("Prêt pour l'inférence sur GPU T4 !")
...
# Ground Truth
print("Display of the Ground Truth...")
show_sample(sample=sample, GT=True)
...
# The model `retinanet_FL` was previously moved to CUDA in cell `dHVvgvzUU5B4`.
# We need to ensure it stays on CUDA because `show_sample` expects CUDA input for inference.
# Check the model's device and move it to CUDA if it's not already there.
if next(retinanet_FL.parameters()).is_cuda == False:
    print("Model found on CPU, moving to CUDA...")
    retinanet_FL.cuda()
else:
    print("Model already on CUDA.")

retinanet_FL.eval()

sample = dataset_val[123]

sample['img'] = sample['img'].cpu().float()

if len(sample['img'].shape) == 4:
    sample['img'] = sample['img'].squeeze(0)

print("Inférence finale sur GPU... (Ça arrive !)")
with torch.no_grad():
    show_sample(sample=sample, GT=False, model=retinanet_FL)
...
torch.save(retinanet, '/content/drive/MyDrive/model_final_BCE.pt')